...
const loanOut = function(amount) {
  return new Promise((resolve, reject) => {
    if (creditLimit === 0) {
      reject(`Get Out from here! You have only $${creditLimit}`);
    } else if (amount > creditLimit && creditLimit !== 0) {
      resolve(creditLimit);
    } else {
      resolve(amount);
    }
...
loanOut(150)
  .then(amountReceived => {
    console.log(
      `\t-> I got $${amountReceived} loan from the bank! Remaining Credit Limit: $${creditLimit - amountReceived}`
    );
  })
  .catch(err => {
